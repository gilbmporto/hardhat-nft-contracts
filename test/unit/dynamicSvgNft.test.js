...
!developmentChains.includes(network.name)
  ...
  : describe("Dynamic SVG NFT Unit Tests", function () {
    let dynamicSvgNft, deployer, ethUsdAggregatorMock
    ...
    beforeEach(async () => {
      ...
      ethUsdAggregatorMock = await ethers.getContract("MockV3Aggregator")
    })
    ...
    describe("Constructor", () => {
      it("Sets the starting values correctly", async () => {
        ...
        const ethPriceInUsd = await dynamicSvgNft.getLatestPrice()
        expect(lowSVG).to.equal(lowSVGImageUri)
        expect(highSVG).to.equal(highSVGImageUri) 
        expect(priceFeed).to.equal(ethUsdAggregatorMock.address)
        console.log(ethPriceInUsd.toString() / 1e18)
      })
    })
    ...
    describe("mintNft", () => {
      it("Emits an event and creates the NFT", async () => {
        ...
        expect(tokenURI.toString()).to.equal(highTokenUri)
      })
      it("Token uri keeps being lower when the price doesn't surpass the highvalue", async () => {
        ...
        const tokenURI = await dynamicSvgNft.tokenURI(0)
        expect(tokenURI.toString()).to.equal(lowTokenUri)
      })
    })
